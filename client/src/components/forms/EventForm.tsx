...
const EventForm = (props: EventFormProps) => {
	...
	const onDateChange = (value: React.ChangeEvent<HTMLInputElement>) => {
		console.log('halp');
		const value2: unknown = value as unknown;
	...
	const onNewUser = (e: React.FormEvent) => {
		const newUser = new User(userFirstName, userLastName, userEmail);
		console.log(newUser);
	...
	function handleResize() {
		const small2 = smallMatch(window.innerWidth);
		const large2 = largeMatch(window.innerWidth);
						...
						{formErrors.date ? (
							<LocalizationProvider dateAdapter={AdapterDayjs}>
								<DateTimePicker
									...
									disablePast={true}
									onChange={(
										value: React.ChangeEvent<HTMLInputElement> | null,
									) => {
										if (value) {
											onDateChange(value);
										}
						...
						<FormControl sx={{ m: 1, width: '100%', mt: 1 }}>
							...
							{formErrors.type ? (
								...
								<Select
									...
									input={
										<OutlinedInput sx={{ position: 'relative', top: '10px' }} />
									}
									renderValue={(selected) => {
										return selected;
									}}

			...
			<Box sx={{ flexGrow: 1, margin: '0px 10%' }}>
				<Grid container spacing={2}>
					<Grid xs={12} md={12} lg={12}>
						<Item>
							{' '}
							<h1 style={{ display: 'inline-list-item' }}>
								Event Organizer is: {thisEvent.organizer.firstName}{' '}
								{thisEvent.organizer.lastName}
							</h1>
							{!openOrg ? (
								<span
									style={dropDownStyle}
						...
						{openOrg && (
							<Item>
								...
								<a
									href={
										'mailto:' +
										thisEvent.organizer.email +
										'?subject=' +
										thisEvent.organizer.firstName
									}
								>
									<strong>
										<em>{thisEvent.name}</em>
									</strong>
								</a>
							</Item>
						)}
						...
						<Grid xs={6} md={6} lg={6}>
							<Item>
								...
								<Box component="ul" aria-labelledby="category-a" sx={{ pl: 2 }}>
									<p>
										<em>{thisEvent.type}</em> ( Level: {thisEvent.intensity})
									</p>
									{thisEvent.intensity === 0 ? (
										<p>Just taking in the nature at no set pace</p>
									) : thisEvent.intensity === 1 ? (
										<p>Looking to get active in the outdoors</p>
									) : thisEvent.intensity === 2 ? (
										<p>Trying to actively enjoy the outside. </p>
									) : thisEvent.intensity === 3 ? (
										<p>We will be moving with purpose. </p>
									) : thisEvent.intensity === 4 ? (
										<p>Let break a sweat in nature!</p>
									) : (
										<p>
											Lets get Adventuring! 21+ (Parental Guidance Required)
										</p>
									)}
								</Box>
							</Item>
						...
						{currentOrganizer.organizerId ? (
							thisEvent.organizer.organizerId === currentOrganizer.organizerId ? (
								<>
									...
									<Grid xs={6} md={6} lg={6}>
										<Item onClick={() => nav('/event/update/' + thisEvent._id)}>
											Edit
										</Item>
									</Grid>
								</>
